import json
from pathlib import Path
import random

galx_path = Path("galx_tags.json")
danbooru_path = Path("tags_danbooru_2022.json")
mal_path = Path("mal_5a250b8b201ace01.json")
pixiv_path = Path("pixiv_1ebde36145d88f26.json")
dlsite_path = Path("dlsite_tags_6b2f3fdf89481b2f.json")

with galx_path.open("r", encoding="utf-8") as f:
    galx_tags_lists = json.load(f)  # [[tag1, tag2], [tag3, tag4], ...]

# too much bias
with danbooru_path.open("r", encoding="utf-8") as f:
    danbooru_tags = json.load(f)  # [tag1, tag2, tag3, tag4, ...]

with mal_path.open("r", encoding="utf-8") as f:
    mal_tags_lists = json.load(f)  # [[tag1, tag2], [tag3, tag4], ...]

with pixiv_path.open("r", encoding="utf-8") as f:
    pixiv_tags_list = json.load(f) # [[tag1, tag2], [tag3, tag4], ...]

with dlsite_path.open("r", encoding="utf-8") as f:
    dlsite_tags_list = json.load(f) # [[tag1, tag2], [tag3, tag4], ...]

flat_galx_tags = [tag for tags in galx_tags_lists for tag in tags]
flat_mal_tags = [tag for tags in mal_tags_lists for tag in tags]
flat_pixiv_tags = [tag for tags in pixiv_tags_list for tag in tags]
flat_dlsite_tags = [tag for tags in dlsite_tags_list for tag in tags]
all_tags_set = set(danbooru_tags) | set(flat_galx_tags) | set(flat_mal_tags) | set(flat_pixiv_tags) | set(flat_dlsite_tags)

# all_tags_set = sorted(all_tags_set)
all_tags_list = list(all_tags_set)

random.shuffle(all_tags_list)
random.shuffle(galx_tags_lists)
random.shuffle(mal_tags_lists)
random.shuffle(pixiv_tags_list)
random.shuffle(dlsite_tags_list)

output = {
    "tags": all_tags_list,
    "real_examples": galx_tags_lists + mal_tags_lists + pixiv_tags_list + dlsite_tags_list
}

out_path = Path("output/merged_tags_v3.json")
with out_path.open("w", encoding="utf-8") as f:
    json.dump(output, f, ensure_ascii=False)

print(
    f"Saved {len(all_tags_set)} unique tags and {len(galx_tags_lists)} examples to {out_path}"
)
